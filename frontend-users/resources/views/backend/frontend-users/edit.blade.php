...
@section('title', __('general.update_frontend_user'))
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('backend.frontend-users.update', ['frontendUser' => $frontendUser->id]) }}" method="POST" autocomplete="off" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                ...
                <div class="row">
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="language_id" id="language_id" class="form-select" required>
                            ...
                            @foreach($languages as $language)
                                <option value="{{ $language->id }}" @selected(old('language_id', $frontendUser->language_id) == $language->id)>
                                    {{ $language->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" class="form-control" id="name" name="name" placeholder="{{ __('general.name') }}" value="{{ old('name', $frontendUser->name) }}" required>
                        @include('backend.partials.input-validation', ['name' => 'name'])
                    </div>

                    <div class="col-md-6 mb-4">
                        <label for="last_name" class="form-label">{{ __('general.last_name') }}</label>
                        <input type="text" class="form-control" id="last_name" name="last_name" placeholder="{{ __('general.last_name') }}"
                               value="{{ old('last_name', $frontendUser->last_name) }}" required>
                        @include('backend.partials.input-validation', ['name' => 'last_name'])
                    </div>

                    <div class="col-md-6 mb-4">
                        <label for="email" class="form-label">{{ __('general.email') }}</label>
                        <input type="email" class="form-control" id="email" name="email" placeholder="{{ __('general.email') }}" value="{{ old('email', $frontendUser->email) }}"
                               required>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="password" class="form-control" id="password" name="password" placeholder="{{ __('general.password') }}">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="password" class="form-control" id="password_confirmation" name="password_confirmation" placeholder="{{ __('general.password_confirmation') }}">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="is_active" id="is_active" class="form-select" required>
                            <option value="1" @selected(old('is_active', $frontendUser->is_active) == '1')>{{ __('general.active') }}</option>
                            <option value="0" @selected(old('is_active', $frontendUser->is_active) == '0')>{{ __('general.inactive') }}</option>
                        </select>
                        @include('backend.partials.input-validation', ['name' => 'is_active'])
                    </div>

                    <div class="col-md-12">
                        <button class="btn btn-primary w-md me-3">{{ __('general.update_frontend_user') }}</button>
